...
<div class="modal fade" id="empAddModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
  ...
  <div class="form-group">
	...
	function getDepts(ele){
		...
		$.ajax({
			...
			success:function(result){
				...
				//显示部门信息在下拉列表中
				//可以不用id来找 $("#empAddModal select").append("")
		...
		function validate_add_form(){
				...
				if(!regName.test(empName)){
					...
					show_validate_msg("#empName_add_input","error","前台验证用户名可以是2-5位中文或者6-16位英文和数字的组合");
					flag =1;
					//return false;
				}else{
				...
				if(!regEmail.test(email)){
					//alert("邮箱格式不正常");
					show_validate_msg("#email_add_input","error","前台验证邮箱格式不正常");
					flag=1;
					//return false;
				}else{
	...
	//1）.我们可以在创建按钮的时候绑定点击事件   2）.绑定点击.live()
...
$(document).on("click",".edit_btn",function () {
    ...
    $("#emp_update_btn").attr("wzx666",$(this).attr("wzx"));
...
//点击更新，更新员工信息
		$("#emp_update_btn").click(function () {
			...
			if(!regEmail.test(email)){
				//alert("邮箱格式不正常");
				show_validate_msg("#email_update_input","error","前台验证邮箱格式不正常");

				return false;
			}else{
				show_validate_msg("#email_update_input","success","");
                update($(this).attr("wzx666"));
			}
        });
			//2.发送ajax请求 保存更新的员工数据
			//var m1= $(this).attr("edit-id");
    function update(md) {
				$.ajax({
					url:"${APP_PATH}/emp/"+md,
					dataType: false,
					type:"PUT",
					data:$("#empUpdateModal form").serialize()+"&empId="+md,
					success:function (result) {
						//alert(result.msg);
                        //1.关闭对话框
